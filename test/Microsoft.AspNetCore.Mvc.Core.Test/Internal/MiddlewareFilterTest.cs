...
namespace Microsoft.AspNetCore.Mvc.Internal
{
    public class MiddlewareFilterTest
    {
        ...
        [Fact]
        public async Task MiddlewareFilterDoesNotTrytoWriteToResponseIfAFilterAfterAlreadyWroteToIt()
        {
            ...
            Pipeline1.ConfigurePipeline = (appBuilder) =>
            {
                appBuilder.Use(async (httpContext, next) =>
                {
                    ...
                    {
                        httpContext.Response.Headers.Add("h1", "h1-value");
            ...
            var resourceFilter1 = new TestResourceFilter(TestResourceFilterAction.Passthrough);
            var middlewareResourceFilter = new MiddlewareFilter(middlewarePipeline);
            var exceptionThrowingResourceFilter = new TestResourceFilter(TestResourceFilterAction.ShortCircuit);
            ...
            // Assert
            var response = resourceFilter1.ResourceExecutedContext.HttpContext.Response;
            response.Body.Seek(0, SeekOrigin.Begin);
            var streamReader = new StreamReader(response.Body);
            //Assert.True(resourceExecutedContext.HttpContext.Response.Headers.ContainsKey(expectedHeader));
            //Assert.True(resourceExecutedContext.Canceled);
            Assert.Equal("Shortcircuited", streamReader.ReadToEnd());
            Assert.False(invoker.ControllerFactory.CreateCalled);
        }
